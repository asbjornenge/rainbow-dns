...
var records = {
    'a.lol.test.domain.com' : {
        ...
        ipv6 : ["2605:f8b0:4006:802:0:0:0:1010"],
        ttl  : 30
    },
...
describe('Matcher', function() {
    ...
    it('Should match direct queries and return all IPs', function() {
        var results = queryMatcher(records, 'a.lol.test.domain.com', 'ipv4')
        assert(results.length == 2)
    })

    it('Should support group queries', function() {
        var results = queryMatcher(records, 'test.domain.com', 'ipv4')
    ...
    it('Should support wildcard queries', function() {
        var results = queryMatcher(records, '*.lol.test.domain.com', 'ipv4')
    ...
    it('Should support AAAA queries too', function() {
        var results = queryMatcher(records, 'test.domain.com', 'ipv6')
        assert(results.length == 1)
    })
